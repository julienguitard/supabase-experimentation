...
const tokenizer = createTokenizer(tokenEncoder,textCoder);
...
Deno.serve(async (req:Request)=>{
  ...
  // Step 05: Translate the query result to database query DTO
    console.log(`[${Date.now()}] Step 05: Translating query result to database query DTO...`);
    const dbQueryDTO2 = translateDBResponseDTOToDBQueryDTO(queryResult, edgeFunction, 'insert-fragments');//TO DO remove
    //const dbQueryDTO2 = translateQueryResultDTOToDBQueryDTO(tokenizer,queryResult);
    console.log(`[${Date.now()}] Step 05 complete: Database query DTO:`, dbQueryDTO2);

  // Step 06: Compile the database query DTO to actual database query
    console.log(`[${Date.now()}] Step 06: Compiling database query...`);
    const dbQuery2 = compileToDBQuery(supabaseClient, dbQueryDTO2);
    console.log(`[${Date.now()}] Step 06 complete: Compiled query:`, dbQuery2);

  // Step 07: Execute the database query
    console.log(`[${Date.now()}] Step 07: Executing database query...`);
    const queryResult2 = await executeDBQuery(dbQuery2);
    console.log(`[${Date.now()}] Step 07 complete: Query result:`, queryResult2);

  // Step 08: Format the result to response DTO
    console.log(`[${Date.now()}] Step 08: Formatting result to response DTO...`);
    const responseDTO = formatToResponseDTO(queryResult);//TO DO remove
    //const responseDTO = formatToResponseDTO(queryResult2);
    console.log(`[${Date.now()}] Step 08 complete: Response DTO:`, responseDTO);
    
  // Step 09: Create and return the final response
    console.log(`[${Date.now()}] Step 09: Creating final response...`);
