...
create table modified_questions (
  ...
  constraint modified_questions_question_id_fkey foreign key (question_id) references questions (id),
...
create table tmp_modified_questions_insert as (
  select
    id, created_at, question_id, '0afe' as hex_modified_question, user_id
  from
    modified_questions
  where
    false
