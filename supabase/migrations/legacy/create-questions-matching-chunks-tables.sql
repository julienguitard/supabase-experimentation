...
create table tmp_questions_matching_chunks_insert as (
  select
    id, created_at, match_id, chunk_id, user_id
  from
...
drop table if exists tmp_questions_matching_chunks_stg;

create table tmp_questions_matching_chunks_stg as (
  select
    qmc.id, qmc.created_at, m.question_id, qmc.chunk_id, qmc.user_id
  from
    (select * from questions_matching_chunks) as qmc join (select id as match_id, question_id from matches) as m on qmc.match_id = m.match_id
  where
    false
);
